...
class FormModel extends Model
{
    ...
    Public function voteLst($tableName) {
        ...
        $page = new \Think\Page($totalRecord, $perpage);
        return  array(
            'data' => $model->field('a.*,b.nickname,b.company_name,b.mobile,b.address,b.openid')->alias('a')->join('LEFT JOIN wx_user b ON a.uid_id=b.id')->where($where)->limit($page->firstRow, $page->listRows)->order($order)->select(),
            'page' => $page->show(), // 翻页的字符串
            'currentPage' => $currentPage,
            'total' => $totalRecord,
            'perpage' => $perpage,
